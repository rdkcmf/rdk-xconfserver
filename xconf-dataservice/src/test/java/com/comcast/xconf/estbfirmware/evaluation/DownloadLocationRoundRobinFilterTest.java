...
public class DownloadLocationRoundRobinFilterTest {
    ...
    @Test
    public void verifyHttpWhenRCDL() throws Exception {
        FirmwareConfigFacade config = createConfig(DownloadProtocol.tftp);
    ...
    @Test
    public void verifyFullHttpWhenRcdlAndSupportFullUrl() throws Exception {
        FirmwareConfigFacade config = createConfig(DownloadProtocol.tftp);
    ...
    @Test
    public void verifyTftpWhenNoRCDL() throws Exception {
        ...
        Assert.assertTrue(result);
        Assert.assertEquals(IP_ADDRESS, config.getFirmwareLocation());
        Assert.assertEquals(DownloadProtocol.tftp.toString(), config.getFirmwareDownloadProtocol());
    }

    @Test
    public void verifyTftpWhenNeverUseHttp() throws Exception {
        FirmwareConfigFacade config = createConfig(DownloadProtocol.http);
        EstbFirmwareContext.Converted context = createContext(RCDL.toString());
        DownloadLocationRoundRobinFilterValue filter = createFilter();
        filter.setNeverUseHttp(true);

        boolean result = DownloadLocationRoundRobinFilter.filter(config, filter, context);
