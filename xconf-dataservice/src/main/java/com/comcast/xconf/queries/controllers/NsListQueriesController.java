...
public class NsListQueriesController {
    ...
    public ResponseEntity saveNamespacedList(@RequestBody NamespacedList macList) {
        if (StringUtils.isEmpty(macList.getId()) || !macList.getId().matches("^[a-zA-Z0-9]+$")) {
            return new ResponseEntity<>("List name is invalid.", HttpStatus.BAD_REQUEST);
        }

        try {
            GenericNamespacedList listToSave = GenericNamespacedListsConverter.convertFromNamespacedList(macList);
            if (genericNamespacedListQueriesService.getOneByType(listToSave.getId(), GenericNamespacedListTypes.MAC_LIST) != null) {
            ...
            return new ResponseEntity<>("Could not save NamespacedList: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        } catch (Exception e) {
