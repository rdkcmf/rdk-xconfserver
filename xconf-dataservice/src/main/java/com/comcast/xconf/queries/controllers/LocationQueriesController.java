...
public class LocationQueriesController extends BaseQueriesController {
    ...
    public ResponseEntity updateLocationFilter(@RequestBody DownloadLocationFilterWrapper locationFilter, @RequestParam(required = false) String applicationType) {
        ...
        try {
            ...
            log.error("Unable to save location filter", e);
            return new ResponseEntity<>("Unable to save location filter: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity updateDownloadLocationRoundRobinFilter(@RequestBody DownloadLocationRoundRobinFilterValue filter, @RequestParam(required = false) String applicationType) {
        ...
        if (CollectionUtils.isNotEmpty(filter.getRogueModels())) {
            for (Model model : filter.getRogueModels()) {
                if (model != null && !modelService.isExistModel(model.getId())) {
                    return new ResponseEntity<>("Model " + model.getId() + " is not exist", HttpStatus.BAD_REQUEST);
                }
            }
        ...
        try {
            locationFilterService.normalizeFirmwareVersions(filter);
