 ...
 */
@RestController
public class EnvironmentQueriesController {
    ...
    public ResponseEntity createEnvironment(@RequestBody Environment env) {
        if (StringUtils.isEmpty(env.getId()) || !env.getId().matches("^[a-zA-Z0-9]+$")) {
            return new ResponseEntity<>("Environment name is invalid.", HttpStatus.BAD_REQUEST);
        }

        Environment existed = environmentDAO.getOne(env.getId());
        if (existed != null) {
            return new ResponseEntity<>("Environment with current name exists.", HttpStatus.BAD_REQUEST);
        }
        ...
        try {
            ...
            return new ResponseEntity<>("Couldn't save Environment: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
