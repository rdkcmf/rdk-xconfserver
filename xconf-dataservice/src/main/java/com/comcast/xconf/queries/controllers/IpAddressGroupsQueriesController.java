...
public class IpAddressGroupsQueriesController {
    ...
    public ResponseEntity saveIpAddressGroup(@RequestBody IpAddressGroupExtended group) {
        if (StringUtils.isEmpty(group.getName()) || !group.getName().matches("^[-a-zA-Z0-9_.' ]+$")) {
            return new ResponseEntity<>("Group name is invalid.", HttpStatus.BAD_REQUEST);
        }

        try {
            final GenericNamespacedList ipList = GenericNamespacedListsConverter.convertFromIpAddressGroupExtended(group);
            if (genericNamespacedListQueriesService.getOneByType(ipList.getId(), GenericNamespacedListTypes.IP_LIST) != null) {
            ...
            return new ResponseEntity<>("Couldn't save: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        } catch (Exception e) {
    ...
    public ResponseEntity addListData(@PathVariable String listId, @RequestBody StringListWrapper ipList) {
        try {
            ...
            log.error("Could not save IpAddressGroup", e);
            return new ResponseEntity<>("Could not save IpAddressGroup: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        } catch (Exception e) {
            log.error("Could not save IpAddressGroup", e);
            return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
        }
