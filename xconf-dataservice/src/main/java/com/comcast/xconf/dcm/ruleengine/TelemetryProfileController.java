...
public class TelemetryProfileController {
    ...
    @Autowired
    private ISimpleCachedDAO<Rule, TelemetryProfile> temporaryTelemetryProfileDAO;
    ...
    public ResponseEntity createTelemetryEntryFor(
            ...
            @RequestBody final TelemetryProfile profile) throws ValidationException {
    ...
    public List<TelemetryProfile> dropTelemetryEntryFor(
            @PathVariable("contextAttributeName") final String contextAttributeName,
            @PathVariable("expectedValue") final String expectedValue) throws ValidationException {
    ...
    @ExceptionHandler
    @ResponseStatus(value = HttpStatus.BAD_REQUEST, reason = "invalid Telemetry object supplied")
    public void handleValidationException(final ValidationException ve) {
        log.error("", ve);
    }
