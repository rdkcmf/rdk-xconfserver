...
public class Utils {
    ...
    public static <K, T extends IPersistable> List<T> getPage(
            final ISimpleCachedDAO<K, T> dao, final int page, final int pageSize) {
        ...
        for (T value : set) {
            ...
            result.add(CoreUtil.clone(value));
    ...
    public static <T extends IPersistable> List<T> getPageAndUpdateEntityId(
            final IADSSimpleDAO<String, T> dao, final int page, final int pageSize) {

        final Map<String, T> data = dao.getRowsAsMap(null, Integer.MAX_VALUE/100, false);

        updateMapEntityIds(data);

        Set<T> set = new TreeSet<T>(data.values());
    ...
    public static <K, T extends IPersistable> int getNumberOfPages(
            final ISimpleCachedDAO<K, T> dao, final int pageSize) {
