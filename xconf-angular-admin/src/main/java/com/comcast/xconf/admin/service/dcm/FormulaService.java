...
public class FormulaService extends AbstractApplicationTypeAwareService<DCMGenericRule> {
    ...
    @Override
    public DCMGenericRule create(DCMGenericRule formula) throws ValidationException {
    ...
    @Override
    public DCMGenericRule update(DCMGenericRule formula) throws ValidationException {
    ...
    public List<DCMGenericRule> changePriorities(String formulaId, Integer newPriority) throws ValidationException {
        final DCMGenericRule formulaToUpdate = getEntityDAO().getOne(formulaId);
        if (formulaToUpdate == null) {
            throw new EntityNotFoundException("Formula with id " + formulaId + " does not exist");
        }
    ...
    @Override
    public DCMGenericRule delete(final String id) throws ValidationException {
    ...
    public DCMRuleWithSettings importFormula(DCMRuleWithSettings formulaWithSettings, Boolean overwrite) throws ValidationException {
    ...
    public Map<String, List<String>> importFormulas(List<OverwriteWrapper<DCMRuleWithSettings>> wrappedFormulasWithSettings) throws ValidationException {
    ...
    private void saveAll(List<DCMGenericRule> formulasList) throws ValidationException {
        for (DCMGenericRule formula : formulasList) {
            getEntityDAO().setOne(formula.getId(), formula);
        }
    }

    private void saveEntity(AbstractService service, IPersistable persistable, boolean overwrite) throws ValidationException {
        if (persistable != null) {
            if (overwrite) {
    ...
    @Override
    public ISimpleCachedDAO<String, DCMGenericRule> getEntityDAO() {
        return dcmRuleDAO;
    }
    ...
    protected List<Predicate<DCMGenericRule>> getPredicatesByContext(Map<String, String> context) {
        List<Predicate<DCMGenericRule>> predicates = new ArrayList<>();
        if (context == null || context.isEmpty()) {
            return predicates;
        }

        Predicate<DCMGenericRule> xRulePredicate = predicateManager.getXRulePredicate(context);
        if (xRulePredicate != null) {
            predicates.add(xRulePredicate);
        }
        if (context.containsKey(SearchFields.NAME)) {
            predicates.add(predicateManager.new XRuleNamePredicate<DCMGenericRule>(context.get(SearchFields.NAME)));
        }
        return predicates;
    }
}
