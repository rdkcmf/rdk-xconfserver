...
@RestController
@RequestMapping(value = "/firmwarerule", produces = MediaType.APPLICATION_JSON_VALUE)
public class FirmwareRuleController extends AbstractController<FirmwareRule> {
    ...
    public ResponseEntity getFiltered(
        ...
        List<FirmwareRule> firmwareRules = firmwareRuleService.findByContext(searchContext);
        Collections.sort(firmwareRules);
        HttpHeaders headers = new HttpHeaders();
        putSizesOfRulesByTypeIntoHeaders(headers, firmwareRules);
        if (MapUtils.isNotEmpty(searchContext) && StringUtils.isNotBlank(searchContext.get(SearchFields.APPLICABLE_ACTION_TYPE))) {
            ApplicableAction.Type type = ApplicableAction.Type.valueOf(searchContext.get(SearchFields.APPLICABLE_ACTION_TYPE));
            firmwareRules = firmwareRuleService.filterByType(firmwareRules, type);
        }
    ...
    public ResponseEntity exportAllByType(@RequestParam String type) {
        ...
        List<FirmwareRule> entities = firmwareRuleService.filterByType(actionType, currentApplicationType);
    ...
    private HttpHeaders putSizesOfRulesByTypeIntoHeaders(final HttpHeaders headers, final List<FirmwareRule> allRules) {
        List<FirmwareRule> filteredByRuleType = firmwareRuleService.filterByType(allRules, ApplicableAction.Type.RULE);
        List<FirmwareRule> filteredByDefinePropertiesType = firmwareRuleService.filterByType(allRules, ApplicableAction.Type.DEFINE_PROPERTIES);
        List<FirmwareRule> filteredByBlockingFilterType = firmwareRuleService.filterByType(allRules, ApplicableAction.Type.BLOCKING_FILTER);
