...
public class LogController {
    ...
    public static final String URL_MAPPING = "/log";

    @Autowired
    IListingDAO<String, String, LastConfigLog> lastConfigLogDAO;

    @Autowired
    IListingDAO<String, String, ConfigChangeLog> configChangeLogDAO;
    ...
    public ResponseEntity getLogs(@PathVariable String macStr) {
        ...
        LastConfigLog last = lastConfigLogDAO.getOne(macAddress.toString(), LastConfigLog.LAST_CONFIG_LOG_ID);
        if (last != null) {
            result.put("lastConfigLog", last);
            result.put("configChangeLog", configChangeLogDAO.getAll(macAddress.toString(), ConfigChangeLog.FILTER_LAST_CONFIG));
        }
