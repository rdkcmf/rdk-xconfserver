...
public class StatisticsController {
    ...
    public static final String URL_MAPPING = "stats";
    ...
    public Map<String, CacheInfo> reloadAllCache() {
        CacheManager.refreshAll();
        Map<String, CacheInfo> statistics = statistics();
        LoggingUtils.log(logger, Level.INFO, "Reloaded All cache: {}", CoreUtil.toJSON(statistics));
        return statistics;
    }

    @RequestMapping(value = "/cache/{cfname}/reload", method = RequestMethod.GET)
    public CacheInfo reloadCacheByCfName(@PathVariable("cfname") final String cfName) {
        CacheManager.refreshAll(cfName);
        CacheInfo cacheInfo = CacheManager.getCacheMBeans().get(cfName);
        LoggingUtils.log(logger, Level.INFO, "Reloaded cache for {}: {}", cfName, CoreUtil.toJSON(cacheInfo));
        return cacheInfo;
    }
