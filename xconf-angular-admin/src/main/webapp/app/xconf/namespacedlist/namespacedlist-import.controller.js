    ...
    function controller($scope, $log, $modal, $stateParams, alertsService, namespacedListService, importService, utilsService, NAMESPACED_LIST_TYPE, paginationService, ENTITY_TYPE) {
        ...
        function retrieveFile(fileName) {
            ...
            importService.openFile(fileName, null, this).then(function (result) {
                vm.namespacedLists = getNamespacedListsFromFile(result);
                if (vm.namespacedLists) {
                    ...
                    vm.wrappedNamespacedLists = [];
                    angular.forEach(vm.namespacedLists, function(val, key) {
                        var wrappedNamespacedList = {};
                        wrappedNamespacedList.entity = val;
                        wrappedNamespacedList.overwrite = false;
                        vm.wrappedNamespacedLists.push(wrappedNamespacedList);
                    });
                }
                vm.isOverwritten = false;
                selectPage();
            }, function (reason) {
                vm.mainErrorMessage = reason.data.message;
            });
        }
