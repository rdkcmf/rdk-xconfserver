...
(function() {
    ...
    function controller($scope, $log, $modal, $location, alertsService, firmwareRuleTemplateService, importService, utilsService, APPLICABLE_ACTION_TYPE) {
        ...
        function init(file) {
            var firmwareRuleTemplates = getFirmwareRuleTemplatesFromFile(file);
            utilsService.sortObjectsByPriority(firmwareRuleTemplates);
            vm.wrappedFirmwareRuleTemplates = wrapFirmwareRuleTemplates(firmwareRuleTemplates);
        ...
        function wrapFirmwareRuleTemplates(firmwareRuleTemplates) {
            var wrappedFirmwareRuleTemplates = [];
            angular.forEach(firmwareRuleTemplates, function(val, key) {
                var wrappedFirmwareRuleTemplate = {};
                wrappedFirmwareRuleTemplate.entity = val;
                wrappedFirmwareRuleTemplate.overwrite = false;
                wrappedFirmwareRuleTemplates.push(wrappedFirmwareRuleTemplate);
            });

            return wrappedFirmwareRuleTemplates;
        }

        function getFirmwareRuleTemplatesFromFile(data) {
            try {
                return JSON.parse(data);
            } catch(e) {
                alertsService.showError({title: 'JSONStructureException', message: 'FirmwareRuleTemplates JSON has some errors! Please, check this file!'});
                $log.error('FirmwareRuleTemplates JSON file is invalid! Please, check it!');
            }
