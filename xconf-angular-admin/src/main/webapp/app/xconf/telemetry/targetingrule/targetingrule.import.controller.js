    ...
    function controller($scope, $log, alertsService, utilsService, importService, targetingRuleService, PermanentProfileService, paginationService) {
        ...
        function retrieveFile(fileName) {
            ...
            importService.openFile(fileName, null, this).then(function (result) {
                vm.targetingRules = getTargetingRulesFromFile(result);
                utilsService.sortObjectsById(vm.targetingRules);
                vm.wrappedTargetingRules = [];
                angular.forEach(vm.targetingRules, function (val, key) {
                    var wrappedTargetingRule = {};
                    wrappedTargetingRule.entity = val;
                    wrappedTargetingRule.overwrite = false;
                    vm.wrappedTargetingRules.push(wrappedTargetingRule);
                });
                vm.isOverwritten = false;
                selectPage();
            }, function (reason) {
        ...
        function getTargetingRulesFromFile(data) {
            try {
                var targetingrules = JSON.parse(data);
                return targetingrules;
            } catch (e) {
                alertsService.showError({
                    title: 'JSONStructureException',
                    message: 'Targeting Rules JSON has some errors! Please, check this file!'
                });
                $log.error('error', e);
            }
