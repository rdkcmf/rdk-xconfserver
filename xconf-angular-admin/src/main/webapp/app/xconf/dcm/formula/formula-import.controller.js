...
(function() {
    ...
    function controller($scope, $log, $modal, $location, alertsService, formulaService, importService, utilsService, paginationService, MODES_TO_GET_LOG_FILES, uploadRepositories) {
        ...
        function init(file) {
            var formulasWithSettings = getFormulasFromFile(file);
            ...
            if (validationResult) {
                ...
                vm.wrappedFormulasWithSettings = [];
                angular.forEach(vm.formulasWithSettings, function(val, key) {
                    var wrappedFormulaWithSettings = {};
                    wrappedFormulaWithSettings.entity = val;
                    wrappedFormulaWithSettings.overwrite = false;
                    vm.wrappedFormulasWithSettings.push(wrappedFormulaWithSettings);
                });
                vm.isOverwritten = false;
                selectPage();
            }
        ...
        function getFormulasFromFile(data) {
            try {
                return JSON.parse(data);
            } catch(e) {
                alertsService.showError({title: 'JSONStructureException', message: 'Formulas JSON has some errors! Please, check this file!'});
                $log.error('Formulas JSON file is invalid! Please, check it!');
            }
