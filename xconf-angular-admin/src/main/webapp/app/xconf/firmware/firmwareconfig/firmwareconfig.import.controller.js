    ...
    function controller($rootScope, $scope, $log, alertsService, utilsService, importService, firmwareConfigService, $modal, paginationService, authUtils, PERMISSION) {
        ...
        function retrieveFile(fileName) {
            ...
            importService.openFile(fileName, null, this).then(function (result) {
                vm.firmwareConfigs = getFirmwareConfigsFromFile(result);
                utilsService.sortObjectsById(vm.firmwareConfigs);
                vm.wrappedFirmwareConfigs = [];
                angular.forEach(vm.firmwareConfigs, function(val, key) {
                    var wrappedFirmwareConfig = {};
                    wrappedFirmwareConfig.entity = val;
                    wrappedFirmwareConfig.overwrite = false;
                    vm.wrappedFirmwareConfigs.push(wrappedFirmwareConfig);
                });
                vm.isOverwritten = false;
                selectPage();
            }, function (reason) {
        ...
        function getFirmwareConfigsFromFile(data) {
            try {
                var firmwareConfigs = JSON.parse(data);
                return firmwareConfigs;
            } catch(e) {
                alertsService.showError({title: 'JSONStructureException', message: 'Firmware config JSON has some errors! Please, check this file!'});
                $log.error('error', e);
            }
