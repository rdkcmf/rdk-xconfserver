...
(function() {
    ...
    function controller($rootScope, $scope, $log, $modal, $location, alertsService, firmwareRuleService, importService, utilsService, APPLICABLE_ACTION_TYPE, paginationService, authUtils, PERMISSION) {
        ...
        function init(file) {
            var firmwareRules = getFirmwareRulesFromFile(file);
            utilsService.sortObjectsById(firmwareRules);
            vm.wrappedFirmwareRules = wrapFirmwareRules(firmwareRules);
        ...
        function wrapFirmwareRules(firmwareRules) {
            var wrappedFirmwareRules = [];
            angular.forEach(firmwareRules, function(val, key) {
                var wrappedFirmwareRule = {};
                wrappedFirmwareRule.entity = val;
                wrappedFirmwareRule.overwrite = false;
                wrappedFirmwareRules.push(wrappedFirmwareRule);
            });

            return wrappedFirmwareRules;
        }

        function getFirmwareRulesFromFile(data) {
            try {
                return JSON.parse(data);
            } catch(e) {
                alertsService.showError({title: 'JSONStructureException', message: 'FirmwareRules JSON has some errors! Please, check this file!'});
                $log.error('FirmwareRules JSON file is invalid! Please, check it!');
            }
