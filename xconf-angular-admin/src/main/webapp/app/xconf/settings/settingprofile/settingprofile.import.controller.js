    ...
    function controller($scope, $log, alertsService, utilsService, importService, settingProfileService, paginationService) {
        ...
        function retrieveFile(fileName) {
            ...
            importService.openFile(fileName, null, this).then(function (result) {
                vm.settingProfiles = getSettingProfilesFromFile(result);
                utilsService.sortObjectsById(vm.settingProfiles);
                vm.wrappedSettingProfiles = [];
                angular.forEach(vm.settingProfiles, function (val, key) {
                    var wrappedSettingProfile = {};
                    wrappedSettingProfile.entity = val;
                    wrappedSettingProfile.overwrite = false;
                    vm.wrappedSettingProfiles.push(wrappedSettingProfile);
                });
                vm.isOverwritten = false;
                selectPage();
            }, function (reason) {
        ...
        function getSettingProfilesFromFile(data) {
            try {
