    ...
    function controller($scope, $log, alertsService, utilsService, importService, settingRuleService, settingProfileService, paginationService) {
        ...
        function retrieveFile(fileName) {
            ...
            importService.openFile(fileName, null, this).then(function (result) {
                vm.settingRules = getSettingRulesFromFile(result);
                utilsService.sortObjectsById(vm.settingRules);
                vm.wrappedSettingRules = [];
                angular.forEach(vm.settingRules, function (val, key) {
                    var wrappedSettingRule = {};
                    wrappedSettingRule.entity = val;
                    wrappedSettingRule.overwrite = false;
                    vm.wrappedSettingRules.push(wrappedSettingRule);
                });
                vm.isOverwritten = false;
                selectPage();
            }, function (reason) {
        ...
        function getSettingRulesFromFile(data) {
            try {
                return JSON.parse(data);
            } catch (e) {
                alertsService.showError({
                    title: 'JSONStructureException',
                    message: 'Setting Rules JSON has some errors! Please, check this file!'
                });
                $log.error('error', e);
            }
