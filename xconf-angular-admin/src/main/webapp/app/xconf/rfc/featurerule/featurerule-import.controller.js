    ...
    function controller($scope, featureRuleService, importService, paginationService, utilsService, alertsService, $uibModal) {
        ...
        function retrieveFile(fileName) {
            ...
            importService.openFile(fileName, null, this).then(function (result) {
                featureRules = getFeatureRulesFromFile(result);
                utilsService.sortObjectsById(featureRules);
                vm.wrappedFeatureRules = [];
                angular.forEach(featureRules, function (value) {
                    var wrappedFeatureRule = {};
                    wrappedFeatureRule.entity = value;
                    wrappedFeatureRule.overwrite = false;
                    vm.wrappedFeatureRules.push(wrappedFeatureRule);
                });
                vm.isOverwritten = false;
                selectPage();
            }, function (reason) {
        ...
        function getFeatureRulesFromFile(data) {
            try {
                return JSON.parse(data);
            } catch (e) {
                alertsService.showError({
                    title: 'JSONStructureException',
                    message: 'FeatureRules JSON has some errors! Please, check this file!'
                });
                $log.error('error', e);
            }
        }
