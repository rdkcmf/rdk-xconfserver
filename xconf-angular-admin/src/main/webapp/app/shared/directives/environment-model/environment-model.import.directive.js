                ...
                link: function (scope, element, attrs) {
                    ...
                    function retrieveFile(fileName) {
                        ...
                        importService.openFile(fileName, null, this).then(function (result) {
                            scope.entities = getEntitiesFromFile(result);
                            utilsService.sortObjectsById(scope.entities);
                            scope.wrappedEntities = [];
                            angular.forEach(scope.entities, function(val, key) {
                                var wrappedEntity = {};
                                wrappedEntity.entity = val;
                                wrappedEntity.overwrite = false;
                                scope.wrappedEntities.push(wrappedEntity);
                            });
                            scope.isOverwritten = false;
                            scope.selectPage();
                        }, function (reason) {
                    ...
                    function getEntitiesFromFile(data) {
                        try {
                            ...
                            alertsService.showError({title: 'JSONStructureException', message: 'Entity JSON has some errors! Please, check this file!'});
                            $log.error('error', e);
                        }
                    }
